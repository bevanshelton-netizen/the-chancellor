...
const empty = { audits: [], messages: [], files: [], payments: [] };
...
function insert(collection, record) { const db = read(); const item = { id: id(collection.slice(0,3)), createdAt: new Date().toISOString(), ...record }; db[collection].push(item); write(db); return item; }
function update(collection, recordId, patch) { const db = read(); const index = db[collection].findIndex(x => x.id === recordId); if (index < 0) return null; db[collection][index] = { ...db[collection][index], ...patch, updatedAt: new Date().toISOString() }; write(db); return db[collection][index]; }
