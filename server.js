...
app.get('/api/status', (_, res) => res.json({ liveAI: Boolean(process.env.OPENAI_API_KEY), voice: Boolean(process.env.OPENAI_API_KEY), admin: Boolean(process.env.ADMIN_EMAIL && process.env.ADMIN_PASSWORD), payments: Boolean(process.env.PAYFAST_MERCHANT_ID && process.env.PAYFAST_MERCHANT_KEY) }));
...
app.post('/api/auth/admin', (req, res) => { if (!process.env.ADMIN_EMAIL || !process.env.ADMIN_PASSWORD) return res.status(503).json({ error: 'Administrator access is not configured on this deployment.' }); const suppliedEmail = Buffer.from(normalise(req.body.email)); const expectedEmail = Buffer.from(normalise(process.env.ADMIN_EMAIL)); const emailOk = suppliedEmail.length === expectedEmail.length && crypto.timingSafeEqual(suppliedEmail, expectedEmail); const suppliedPassword = Buffer.from(normalise(req.body.password)); const expectedPassword = Buffer.from(String(process.env.ADMIN_PASSWORD)); const passwordOk = suppliedPassword.length === expectedPassword.length && crypto.timingSafeEqual(suppliedPassword, expectedPassword); if (!emailOk || !passwordOk) return res.status(401).json({ error: 'Sign-in details are incorrect.' }); req.session.admin = true; res.json({ ok: true }); });
