...
async function loadReadinessDefinition(){
  const box=$('#questions');
  if(!box)return;
  box.innerHTML='<p class="muted">Loading the 9-category Business Readiness Audit…</p>';
  try{
    ...
    box.innerHTML=readinessDefinition.map(section=>`<div class="audit-question-group"><h3>${section.name}</h3>${section.questions.map(q=>`<label>${q.text}<select name="${q.id}" required>${answerOptions}</select></label>`).join('')}</div>`).join('');
  }catch(error){
