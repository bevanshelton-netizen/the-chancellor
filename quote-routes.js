...
function publicSuggestion(audit){
  ...
  return {service:q.service,amount:q.amount,description:q.description,deliverables:q.deliverables,primaryPriority:q.primaryPriority,supportingPriorities:q.supportingPriorities,humanReviewRequired:true};
}
...
function createRecommendedOffer(audit){
  ...
  const offer=store.insert('offers',{
    ...
    expiresAt:new Date(Date.now()+Number(q.expiresInDays||7)*86400000).toISOString(),source:'automatic-recommendation',humanReviewed:true
  });
...
module.exports=function registerQuoteRoutes(app){
  app.get('/api/client/quote-recommendation',requireClient,(req,res)=>{
    ...
    const sentOffer=(db.offers||[]).slice().reverse().find(o=>o.auditId===audit.id&&o.source==='automatic-recommendation')||null;
    res.json({recommendation:publicSuggestion(audit),auditPaid:paidAudit(db,audit.id),request,sentOffer});
  });
  ...
  app.post('/api/client/quote-recommendation/request',requireClient,(req,res)=>{
    ...
    const existing=(db.quoteRequests||[]).slice().reverse().find(x=>x.auditId===audit.id&&['Requested','Approved'].includes(x.status));
    if(existing)return res.json({request:existing,recommendation:publicSuggestion(audit)});
    const suggestion=buildQuoteSuggestion(audit);
    const request=store.insert('quoteRequests',{auditId:audit.id,businessName:audit.businessName,clientName:audit.name,clientEmail:audit.email,status:'Requested',suggestion,requestedAt:new Date().toISOString()});
    store.update('audits',audit.id,{salesStage:'Upsell opportunity',recommendedService:suggestion.service,quoteAmount:suggestion.amount,nextAction:'Human review of recommended quotation'});
    res.status(201).json({request,recommendation:publicSuggestion(audit)});
  });
