...
This handoff routes THE CHANCELLOR through the IZAKHONO Core + IZAKHONO Cloud path instead of Vercel.
...
1. Provision a Docker-capable Ubuntu 24.04 host through the IZAKHONO Core/Cloud first-server process.
...
This handoff makes THE CHANCELLOR deployable on the IZAKHONO sovereign path, but it does not by itself prove that the underlying public host is production-ready. Do not claim commercial readiness until the first real host has passed the IZAKHONO production-proof/READY gate and this application has passed its own health, persistence, backup/restore and payment checks.
